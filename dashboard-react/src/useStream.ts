...
function makeLabels(n: number): string[] {
  ...
  return Array.from({ length: n }, (_, i) =>
    new Date(now - (n - 1 - i) * 1000).toLocaleTimeString('en', {
...
export function useStream() {
  const [state, setState] = useState<StreamState>(() => {
    ...
    for (let i = 0; i < N_LINE; i++) {
      cpu = walk(cpu, 5, 95, 8);
      mem = walk(mem, 20, 90, 5);
      net = walk(net, 5, 80, 10);
    ...
    const areaArr = Array.from({ length: N_AREA }, () =>
      parseFloat(randF(0.2, 3.5).toFixed(2))
    );
  ...
  const tick = useCallback(() => {
    setState(prev => {
      ...
      return {
        ...prev, cpu, mem, net,
        lineLabels: newLabels,
        cpuData: newCpu, memData: newMem, netData: newNet,
        barData: newBar, areaData: newArea,
        heatmap: hm, assets, services,
  ...
  const toggleDataset = useCallback((name: 'CPU' | 'Memory' | 'Network') => {
    setState(prev => ({
      ...prev,
      datasets: { ...prev.datasets, [name]: !prev.datasets[name] },
    }));
  }, []);
  ...
  const simulateError = useCallback((msgs: string[]) => {
    ...
    setState(prev => ({
      ...prev,
      errorMsg: msg,
      feedItems: [e, ...prev.feedItems].slice(0, 200),
    }));
