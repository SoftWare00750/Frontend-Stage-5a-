      ...
      >
        ...
        {EXPERIENCE.map((exp: ExperienceItem, i: number) => (
          <div key={i} style={{ position: 'relative', paddingBottom: '3rem', paddingLeft: '2rem' }}>
            {/* Dot */}
            <div style={{
              position: 'absolute', left: '-2.3rem', top: '0.4rem',
              width: 10, height: 10, borderRadius: '50%',
              background: 'var(--accent)', border: '2px solid var(--bg)',
              boxShadow: '0 0 0 3px rgba(124,106,245,0.2)',
            }} />
            ...
            <div style={{
              color: 'var(--accent2)', fontSize: '0.85rem',
              fontWeight: 600, marginBottom: '0.8rem',
            }}>
              {exp.role}
            </div>

            <div style={{ color: 'var(--text2)', fontSize: '0.87rem', lineHeight: 1.8 }}>
              {exp.desc}
            </div>
          </div>
        ))}
